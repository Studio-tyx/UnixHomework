...
/* Socket constructor used to initialize the socket according to the */
/* socket type. A hostname name should only be specified for client sockets. */
{
	Mysocket = -1;
	conn_socket = -1;
	bytes_read = bytes_moved = 0;
	...
	socket_error = MySOCKET_NO_ERROR;
	/* Initialize the socket. NOTE: Any errors detected during initialization */
	/* will be recorded in the socket—error member. */
...
/* Returns a valid socket descriptor or -1 if the socket cannot be initialized. */
{
	address_family = af;
	socket_type = st;
	protocol_family = pf;
	...
	if (hostname) {
		...
		sin.sin_addr.s_addr = INADDR_ANY; /* Use my TP address */
...
/* Create and initialize a socket according to the socket type.  */
/* This cross-platform function will only accept SOCK_STREAM and SOCK_DGRAM */
...
/* Function used to multiplex reads without polling. Returns false if */
/* a reply time is longer then the timeout values. */
{
...
int MySocket::RemoteRecv(void *buf, int bytes, int seconds, int useconds, int flags)
/* Receive a block of data from a remote socket in blocking mode with  */
/* a specified timeout value. Returns the total number of bytes received */
...
/* if an error occurs. */
{
	int rv = listen(Mysocket,  /* Bound socket */
		max_connections);  /* Number of connection in request queue */
...
/* call blocks the server until the client requests  */
...
/* local association. This functjon is especially useful When a  */
...
/* determine the local associat ion which has been set by the system  */
/* Returns -1 it an error occurs. */
{
...
/* Gets the current socket option for the specifiea option */
/* Returns -1 if an error occurs. */
{
...
/* found. NOTE: The calling function must free the memory */
...
/* Function used to obtain service information about specified */
/* The source of this information is dependent on the calling function's */
/* platform configuration which should be a local services file or NIS */
/* database. Returns a pointer to a servent data structure if service  */
/* information is available or a null value if the service cannot be  */
...
/* Returns -J if an unknown service or protocol is requested. NOTE: This */
/* information is obtained from this machines local services file or  */
/* from a NIS database. */
{
	/*If the "protocol" pointer is NULL, getservbyname returns */
	/* the first service entry for which Che name matches the s_name  */
	/* pr one of the s_aljases.l Otherwise getservbyname matches both */
	/* name and the proto. */
	servent *sp = getservbyname(name, protocol);
	if (sp == 0) {
...
/* NTS database. Returns a pointer to a hostent data structure */
/* if information is available or a null value if the hostname cannot */
/* be found. NOTE: The calling function must free the memory allocated  */
...
/* A memory buffer equal "MysMAX_NAME_LEN" to must be pre-allocated  */
/* prior to using this function. Return -1 if an error occurs. */
{
	char hostname[MysMAX_NAME_LEN];
	int rv = GetHostName(hostname);
...
int MySocket::RecvFrom(void *buf, int bytes, int seconds, int useconds, int flags)
/* Receive a block of data from a remote datagram socket  */
/* and do not return until all the bytes have been read */
/* or thé timeout value has been exceeded. Returns the total */
/* number of bytes received or -1 if an error occurs. */
{
...
/* Receive a block of data from a remote datagram socket  */
/* and do not return until all the bytes have been read.  */
/* Returns the total number of bytes received or -1 l if */
/* an error occurs . */
{
...
int MySocket::RecvFrom(int s, sockaddr_in*sa, void *buf,
	int bytes, int seconds, int ugeconds, int flags)
/* Receive a block of data from a remote datagram socket */
/* and do not return until all the bytes have been read */
...
/* number of bytes received or -1 if an error occurs. */
{
	...
	while (bytes_read < bytes) { /* Loop until the buffer is full */
		...
		if ((num_read = recvfrom(s, p, flags,
...
/* Receive a block of data from a remote datagram socket  */
/* and do not return until all the bytes have been read.  */
/* Returns the total number of bytes received or -l if */
/* an error occurs. */
{
	/* Length of client address */
	int addr_size = (int) sizeof(sockaddr_in);
